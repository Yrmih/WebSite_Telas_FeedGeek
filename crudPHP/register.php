...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty(trim($_POST["username"]))) {
    ...
    if (!ctype_alnum(str_replace(array("@", "-", "_"), "", $username))) { //verifica se os caracter são alfanumericos
  ...
  if (empty(trim($_POST["email"]))) {
    ...
    $email = filter_var($_POST["email"], FILTER_SANITIZE_EMAIL); //Sanitiza email para somente caracteres alfanumericos
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) { //valida email segundo padrao RFC822
